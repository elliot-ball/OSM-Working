...
(function(window, $) {
  ...
  TinyAutocomplete.prototype = {
    defaults: {
      ...
      groupTemplate: '<li class="autocomplete-group"><span class="autocomplete-group-header">{{Name}}</span><ul class="autocomplete-items" /></li>',
      itemTemplate: '<row groupid="{{ID_Group}}" fullpath="{{FullPath}}" grouppath="{{GroupPath}}"><a class="isselected" href="#">{{Name}}</a></row>'
    },
    ...
    renderLastItem: function() {
      this.list.append( this.template( this.settings.lastItemTemplate, {Name: this.field.val()} ) );
    },
